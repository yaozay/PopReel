...
import { useAuth } from "@clerk/nextjs"; // Ensure Clerk's Auth is properly set up
import { useRouter } from "next/navigation";

const Sidebar: React.FC = () => {
  const { signOut, isSignedIn } = useAuth(); // Check if user is signed in
  ...
  const handleLogout = async () => {
    try {
      ...
      router.push("/"); // Redirect to the home or login page after logging out
    } catch (error) {
